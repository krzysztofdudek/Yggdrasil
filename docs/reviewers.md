...
- **Either kind — the rule's support files:** every other file in the rule's directory is folded in with the rule file — a helper module `check.mjs` or `companion.mjs` imports, a table it ships — so editing one re-opens every pair of the rule. Left out: `yg-aspect.yaml`, `log.md`, a package rule's adaptation and its log, `provenance.json`, `node_modules`, and every non-code file under a dot-named entry; a file under `drills/` or in a nested rule's directory counts only when the rule's code imports it. For a reviewer rule this includes files the reviewer is never shown: only `content.md` reaches the prompt as the rule (list another file under `references:` to show it). Full list: [The lock](/the-lock#what-makes-a-verdict-valid).
