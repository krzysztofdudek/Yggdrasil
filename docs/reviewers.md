...
- **Rules are code.** They run with the permissions of whoever runs the command: `check.mjs` under `yg check --approve`, `--approve --only-deterministic`, `yg adopt`, `yg aspect-test` and `yg drill`; `companion.mjs` only under `yg check --approve` and `yg aspect-test`. Every other command executes no repository code. For CI on fork pull requests, see [The lock → What `yg check` proves](/the-lock#what-yg-check-proves-and-against-whom).
