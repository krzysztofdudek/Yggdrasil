...
- **Either kind — the rule's support files:** every other file in the rule's directory is folded in with the rule file — a helper module `check.mjs` or `companion.mjs` imports, a table it ships — so editing one re-opens every pair of the rule. Left out: `yg-aspect.yaml`, `log.md`, a package rule's adaptation and its log, `provenance.json`, `node_modules`, and every non-code or gitignored file under a dot-named entry; a file under `drills/` or in a nested rule's directory counts only when the rule's code names it by a literal relative specifier. For a reviewer rule this includes files the reviewer is never shown: only `content.md` reaches the prompt as the rule (list another file under `references:` to show it). Full list: [The lock](/the-lock#what-makes-a-verdict-valid).

`yg check` by default recomputes each pair's input hash and compares it to the lock — no LLM calls, no provider keys, runs instantly. If `auto_approve` is set to `deterministic` or `full` in `yg-config.yaml`, bare `yg check` may fill pairs automatically (see [Configuration](/configuration#auto-approve-config)); explicit CLI flags always override the config, a triage view (`--top`, `--summary`, `--aspect`, `--details`) never fills, and under CI (the `CI` variable set) a committed `full` is held back and the bare check stays read-only. A source edit and an aspect-content edit both surface the same way: the affected pairs no longer match their recorded hash, so check reports them as unverified until `yg check --approve` fills them again.
...
- **Rules are code.** They run with the permissions of whoever runs the command: `check.mjs` under `yg check --approve`, `--approve --only-deterministic`, `yg adopt`, `yg aspect-test`, `yg drill` and `yg simulate` (the candidate, once per replayed commit); `companion.mjs` only under `yg check --approve` and `yg aspect-test`. Every other command executes no repository code. For CI on fork pull requests, see [The lock → What `yg check` proves](/the-lock#what-yg-check-proves-and-against-whom).
...
**Imports inside `check.mjs`.** A helper your check imports from the rule's own directory is part of the rule: it is folded into the rule's hash, followed from module to module (a helper kept under `drills/` or in a nested rule's directory too, once the check imports it), so editing it re-opens the rule's pairs. A module from `node_modules`, or from anywhere outside the rule's directory, is **not** folded in: changing it does not invalidate the pair. Guidance: keep a rule's logic inside its own directory; if you import from outside it, consciously accept that a change there does not re-verify on its own.

**CLI version pinning.** Tree-sitter grammars and helper implementations live inside `@chrisdudek/yg`. The input hash excludes the CLI version itself, but a script verdict that read a syntax tree records the grammar and parser runtime that built it, so an upgrade that ships a different grammar re-opens exactly those verdicts, and no others. A release can also change what a verdict's hash folds, as a deliberate decision its changelog names. Pin the CLI version in CI and raise the pin in a commit of its own (see [The lock](/the-lock)); never edit a rule cosmetically to force a re-verification.
