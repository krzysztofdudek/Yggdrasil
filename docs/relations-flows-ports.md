...
A TypeScript/JavaScript dependency is a construct that names a module by a string literal: a static `import`, a re-export (`export … from`), `import x = require(…)`, `export = require(…)`, `require('…')`, a dynamic `import('…')` in a value position, and `new URL('…', import.meta.url)`. Calls, JSX, `extends` and type references only use what an import already brought in, so they add nothing. A mock registration such as `vi.mock('…')` is a call argument, not an import.

**Type-only references are not dependencies.** The check records modules a file loads at runtime, and TypeScript erases type-only constructs from the JavaScript it emits. So every spelling of a type-only reference is silent: `import type { T } from`, an import whose every named specifier carries an inline `type` and which binds no default or namespace, `export type { … } from`, `export type * from`, `import type X = require(…)`, an `import('…')` in a type position (`typeof import('./m')`, `let v: import('./m').T`, the type side of `as` or `satisfies`, a type argument), a module augmentation `declare module './m' { … }`, triple-slash directives, and JSDoc `import()` types. A clause that also binds a value, such as `import { type A, b }` or `import def, { type A }`, loads the module and gives the edge. Before 6.1.0 the type-position `import()` forms and `import type … = require()` produced edges while `import type` did not. One rule now covers them all.
...
A candidate is then probed the way the compiler probes it. A `.js` specifier finds the `.ts` source it compiles from (`.mjs` → `.mts`, `.cjs` → `.cts`). Other explicit extensions (`.json`, `.css`, `.svg`, `.vue`) name the file itself. An extensionless path gets the source extensions appended. A directory is read through its own `package.json` entry, then its `index.ts|tsx|js|jsx|mjs|cjs`.
