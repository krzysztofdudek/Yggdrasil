...
What counts as a dependency depends on the language, and for two of them the build layout matters:

- **C and C++** depend on each other only through `#include`. A quoted include is looked up next to the including file first. After that, the check uses the include roots from a `compile_commands.json` at the repository root or in `build/`: the `-iquote` and `-I` directories of each translation unit, never `-isystem`. An angle include (`#include <lib/x.h>`) is looked up only under those `-I` roots. Without a compilation database, a quoted include of at least two path segments, such as `"net/socket.h"`, is also looked up at the repository root and under every directory named `include`. A bare `"config.h"` is never looked up that way, and neither is an angle include. A header found under two roots gives no edge. Includes in a preprocessor branch that is constant-false (`#if 0`, `#if (0)`, `#if false`, the `#else` of `#if 1`) are ignored, but every branch that depends on a macro counts. Expect missed dependencies without a compilation database: `-I` roots other than `include/` directories and the repository root, headers generated into the build tree, macro includes, and header-unit `import "x.h";` are not seen. Generate the database (CMake: `-DCMAKE_EXPORT_COMPILE_COMMANDS=ON`; Bazel, Meson and `bear` can write one too) to close most of that gap. A `.h` header is read as C++ when its directory holds C++ files and no `.c` file.
- **PHP** class references resolve through Composer autoload maps: PSR-4 (including the `""` fallback prefix) and PSR-0 from the nearest `composer.json`, then from every `composer.json` in the repository (`vendor/` excluded), so a monorepo's cross-package `use` is seen. A class found in two places gives no edge. Besides `use` imports, class names in class positions (`new`, types, `extends`, `::`, attributes, `instanceof`) are resolved as PHP resolves them, from the file's namespace and imports. `require`/`include` counts only when the path is built from `__DIR__` or `dirname(__FILE__)` plus a literal; a path resolved through `include_path` is not seen. `classmap` and `files` autoloading are not read.
...
### What counts as a TypeScript or JavaScript dependency

A TypeScript/JavaScript dependency is a construct that names a module by a string literal: a static `import`, a re-export (`export … from`), `import x = require(…)`, `export = require(…)`, `require('…')`, an `import('…')` in a value or a type position, a module augmentation `declare module '…'`, and `new URL('…', import.meta.url)`. Calls, JSX, `extends` and type references only use what an import already brought in, so they add nothing. A mock registration such as `vi.mock('…')` is a call argument, not an import.

**Type-only references are dependencies.** A relation records that one component's code depends on another's, not only that it loads it at runtime. A file that compiles only against another component's types breaks when those types change, so every spelling of a type-only reference gives an edge, exactly like a value import, and a missing relation is a blocking `relation-undeclared-dependency`: `import type { T } from`, `import type * as T from`, an import whose every named specifier carries an inline `type`, `export type { … } from`, `export type * from`, `import type X = require(…)`, and an `import('…')` in a type position (`typeof import('./m')`, `let v: import('./m').T`, the type side of `as` or `satisfies`, a type argument). If the architecture allows no relation between the two components, move the shared types into a component both may depend on.

A module augmentation `declare module './m' { … }` merges declarations into `./m` and compiles only against it, so it gives an edge too. A relative name is always an augmentation. A bare name (`declare module '@acme/b'`) is an augmentation in a module file, one with a top-level `import` or `export`, and gives an edge when the name resolves to an in-repo package. In a script file the same header is an ambient declaration: it declares a module's shape instead of depending on one, and stays silent. So does a wildcard pattern such as `declare module '*.css'`.

Two type references are not detected, because they live in comments and the check reads syntax, not comment text: a triple-slash directive (`/// <reference path="…" />`) and a JSDoc import type (`/** @type {import('./m').T} */`). Before 6.1.0, `import type`, `export type` and all-inline `type` clauses were silent while the type-position `import()` forms gave edges.

A specifier resolves to a file in this order. A bundler query suffix (`?raw`, `?worker`) is stripped first.

1. **Relative** (`./x`, `../x`): joined onto the importing file's directory.
2. **Root-absolute** (`/src/x`, the Vite and Next.js convention): joined onto the importing file's package root, the nearest ancestor directory with a `package.json`. With no package root it stays silent.
3. **Package imports** (`#internal/x`): through the `imports` map of the nearest `package.json`.
4. **Bare** (`@/x`, `src/x`, `@acme/b`, `@acme/b/sub`): first the `paths` of the nearest `tsconfig.json` (a JavaScript file also reads `jsconfig.json`), with its `extends` chain applied: the exact key, else the longest wildcard prefix. Otherwise its `baseUrl`. Otherwise an in-repo package whose `package.json` `name` matches, resolved through its `exports`, or its `types`/`main` fields and `index` file. Any other bare specifier is an external package and stays silent, as does any specifier with a scheme (`node:path`).

A candidate is then probed the way the compiler probes it. A `.js` specifier finds the `.ts` source it compiles from, then a declaration file (`.ts`, `.tsx`, `.d.ts`, then `.js`; `.mjs` → `.mts`, `.d.mts`; `.cjs` → `.cts`, `.d.cts`). Other explicit extensions (`.json`, `.css`, `.svg`, `.vue`) name the file itself. An extensionless path gets the source extensions appended, with `.d.ts` after `.ts` and `.tsx`. A directory is read through its own `package.json` entry, then its `index.ts|tsx|d.ts|js|jsx|mjs|cjs`.

When the answer is not unique the check stays silent. That happens when a `paths` pattern has two substitutions that both name an existing file, when two in-repo packages share one `name`, or when a `tsconfig.json` is unreadable or `extends` a relative file that does not exist. A published preset in `extends` (`@tsconfig/node20`) that is not installed is taken to set no `paths` or `baseUrl`.

The imports inside Vue and Svelte components are checked too. A `.vue` or `.svelte` file is read through its `<script>` blocks, with the grammar their `lang` names, and at the component's own line numbers.

A grammar can turn valid syntax into a parse error that swallows a statement. With the grammars shipped today, the known case is a re-export with import attributes (`export … from '…' with { … }`, in TypeScript and JavaScript; see [Known grammar limits](#known-grammar-limits)). Lines inside such an error region are rescanned for import and re-export statements, type-only ones included, so a swallowed import still gives its edge. A clean parse is never rescanned.
...
| C# | master 9150f7d (after 0.23.5) | C# 14 extension blocks (`extension(Widget w) { … }`) and `a?.B = 1` parse. No known gap found. | — |
