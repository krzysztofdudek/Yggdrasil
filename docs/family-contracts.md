...
| Family candidates (`.family-candidates.json`) | no schema id — versioned by `v: 1` | two, each naming itself in `producer` and its test for "without a law" in `gate`: Grain — `grain propose` (`producer: grain`, `gate: no-certified-convention`), which writes it into the proposal's `.yggdrasil/` so `yg adopt` installs it with the graph (`--family-candidates <path>` writes it elsewhere, for a repository that adopted earlier); Yggdrasil — the offline miner `scripts/family-without-law.mjs` in the Yggdrasil repository (`producer: yggdrasil-miner`, `gate: no-narrow-aspect`), which writes it into `.yggdrasil/` | Yggdrasil — `yg advise`, which rejects any `v` it does not name | before 6.0.0 | [this page](/family-contracts) |
