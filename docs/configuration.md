...
- **`type_level`** — Boolean, default `false`. Turns on type-level coverage: a file matched by exactly one classifying type's `when` predicate counts as covered by that type even though it has no node of its own. Committed config only — the gitignored `yg-secrets.yaml` overlay can never change it, since the flag changes what counts as covered/uncovered for `yg check`, and that must be the same for everyone working on the repo rather than something a local override can silently flip. Omitting the key (or the whole `coverage` block) leaves today's file-only coverage exactly as it is.
  ...
  Being type-covered satisfies coverage; it does not by itself mean a rule actually runs. A rule attached to the matched type can be scoped to a whole component (nothing to run it against here), filtered off by its own `when:`, or still draft. `yg check` reports both halves per matched type: which rules actually enforce, and which are attached but do not, each with the reason and how many files it affects. When a rule is grouped (a bundle whose implied rules split across a file-level and a whole-unit half), the block names the split explicitly rather than letting the whole-unit half look silently absent. It also names, once, where a type's implicit parent chain stops (a fork between two parents, an absent or explicitly empty `parents:`, or a cycle) — that chain is where an ancestor type's own defaults come from. Because none of this stops a file from satisfying coverage, a repo can have files that are green with genuinely nothing checking them; `yg check` names every one of them plainly, with samples, rather than leaving that discoverable only by accident. `yg context --file <path>` and `yg owner --file <path>` give the same honest answer for one such file directly, in place of a plain "not covered" — the matched type, the chain, and both the rules that apply and the ones that do not.
