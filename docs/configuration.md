...
[--endpoint]`, whenever the graph gains its first reviewer rule.
...
- **reviewer** — Reviewer configuration; when present, must contain `tiers` with at least one entry. Required only once a [reviewer rule](/glossary#llm) is actually effective in the graph — a script-only / keyless project (script rules only, or none) needs no `reviewer:` section, and `yg check` does not ask for one until a reviewer rule exists. Configured via `yg init` (interactively, or non-interactively with `--provider`); see [Reviewer tiers](#reviewer-tiers) below.
...
parallel: 10                          # Concurrent reviewer-rule reviews (reviewer phase only)
debug: false
...
events:                               # Optional — committed-events opt-in (default off)
  committed_llm: true                 # Commit + share reviewer verification events (default false)
...
(with `required: []`, `type_level: true`, and `excluded` listing the
agent-rules files it wrote plus `.gitattributes`), `debug` and
`auto_approve`, plus the `reviewer:` block once you configure a reviewer. It writes
...
- **`required`** — List of roots. Files under a required root that are not mapped to any node produce an `unmapped-files` error (blocks CI). Default: `["/"]` (the whole repo — reproduces the previous always-map-everything behavior). An explicit empty list `[]` means **require nothing** — every uncovered file (outside `excluded`/nested) becomes a non-blocking `uncovered-advisory` warning and nothing blocks (the code's `-advisory` suffix means non-blocking; it is unrelated to the rule status `advisory`) (pure-advisory adoption: you still see the full uncovered surface, but CI stays green on coverage). The empty list only takes effect when written explicitly; omitting the whole `coverage` block keeps the `["/"]` default. `yg init` writes an explicit `required: []` into a fresh `yg-config.yaml`, so newly-initialized projects start in require-nothing mode (green from the first check) — add a root to `required` for each area whose unmapped files should block.
- **`excluded`** — List of roots. Files under an excluded root are silently ignored regardless of other rules — this holds even when a MORE SPECIFIC required root also matches the same file; exclusion always wins once it matches at all. Exclusion is a global filter, not just a coverage-level rule: it cuts a matched path everywhere Yggdrasil looks, including a node's own `mapping:` entry naming that exact path. See below for the full reach of `excluded`.
...
A file matching ANY excluded root is dropped entirely, before it is ever sorted into the blocking or non-blocking coverage level — exclusion is absolute, independent of whether a required root also matches it and independent of how specific either root is. Among the files that match no excluded root, any matching required root puts the file in the blocking level. A required root fully contained inside an excluded root can therefore never match anything; `yg check` warns (`coverage-required-shadowed`) when both roots are plain (non-glob) paths.

**An empty `required` list means no file can ever fail coverage.** That is the shipped default — `yg init` writes `required: []`, and so does a mined proposal — and it is deliberate: a brownfield repository is green from its first check. Its consequence would be easy to miss, because the uncovered files are listed either way and only their severity differs. So the report says it where the files are listed: the heading of the uncovered block names the reason they never block.

```text
warning[uncovered] 13 files belong to no node — not under coverage.required, so they never block
```

The block is a warning, never an error, and it disappears the moment nothing is left uncovered; name a path under `coverage.required` in `.yggdrasil/yg-config.yaml` and files under it report as `error[unmapped]` instead, blocking until a component owns them.

---

## Prompt-size gate

A tier's optional `max_prompt_chars` caps the length of the prompt the
reviewer assembles for each pair. The prompt for each reviewer pair is composed of:
...
`yg check` measures the assembled prompt for every expected reviewer pair and reports
`prompt-too-large` — a blocking error — when it exceeds the resolved tier's limit.
The check is deterministic and costs nothing; script pairs have no prompt
...
imported attention record (`advise-imported.jsonl`), (when opted in) the shared
...
auto_approve: deterministic   # fill the script-rule cache automatically on bare yg check
```
...
`events` is an optional section that controls where reviewer verification events are
...
| Key | Type | Default | Effect |
| --- | --- | --- | --- |
| `committed_llm` | boolean | `false` | Opt into a committed, team-shared record of reviewer verification events. |

```yaml
# .yggdrasil/yg-config.yaml
events:
  committed_llm: true   # commit + share reviewer verification events (default off)
```

When `committed_llm` is on, each reviewer verification event is appended to a
...
- **Reviewer rules only.** Script checks, drill runs, and diagnostic runs stay
