...
- **`type_level`** — Boolean, default `false`. Turns on type-level coverage: a file matched by exactly one classifying type's `when` predicate counts as covered by that type even though it has no node of its own. Committed config only — the gitignored `yg-secrets.yaml` overlay can never change it, since the flag changes what counts as covered/uncovered for `yg check`, and that must be the same for everyone working on the repo rather than something a local override can silently flip. Omitting the key (or the whole `coverage` block) leaves today's file-only coverage exactly as it is.
  ...
  A file under an `excluded` root is skipped before any of this — never classified, never counted, never reported.
...
- A subtree that is its own separate project is auto-skipped everywhere a **directory or glob** mapping entry SWEEPS it in as the files a node actually enforces or is billed for: the repo-walking coverage checks, expected review pairs, a node's source fingerprint, the dependency-conformance pass's per-node file enumeration, type-`when` classification, and the files/allowances a rule's review actually runs against — `ctx.fs`, the parsers, and companion resolution all refuse a path there too, including one reached through a symlink that resolves into the separate project. `yg context --file` and `yg owner --file` agree: a file a directory or glob entry merely swept into the separate project is never reported as owned by, or bound to satisfy the rules of, a node outside it. "Its own separate project" means a subtree that carries its own nested `.yggdrasil/` graph, or its own `.git` — a fully independent checkout, a submodule, or a linked `git worktree`. This applies identically to a directory mapping and a glob mapping, and is unaffected by whether `.gitignore` hides the marker: the boundary is read off the filesystem, not off any particular mapping's own file listing. A vendored dependency, git worktree, or submodule that carries its own graph or its own git checkout never becomes this graph's enforcement surface just because a directory or glob mapping happens to recurse over it; it is governed by its own project, not the root graph. An ordinary subdirectory with no graph or git checkout of its own — including a dependency directory such as `node_modules` or `vendor` — is unaffected and still gets covered normally; there is no name-based guessing, so excluding a dependency directory by name is the adopter's own `coverage.excluded` config.

  A mapping entry that **names one file exactly** is a different question this boundary never answers: it is the node's own explicit, deliberate ownership claim over that one path, not a directory or glob recursing into files it never specifically named. Every check above leaves an exact entry alone — it stays part of the node's enforcement surface, its own fingerprint, its own read allowances, and its own reported ownership, even when the path it names sits inside a separate project. Whether that explicit claim is itself a valid mapping is a separate matter, decided once, by the `file-mapping-nested-project` check described above — not by the boundary this section documents.

  The one place this boundary is deliberately NOT applied, even for a directory or glob entry, is a mapping entry's own existence check: a glob or exact path that resolves only to a separate project's real, on-disk files is not stale or broken, so `mapping-path-missing` stays silent about which project owns that content. What it does report is the aggregate case — a node whose DIRECTORY or GLOB entries, taken as a whole, resolve to nothing this node can enforce because every file they reached belongs to a separate project (an exact entry is never part of this computation, for the same reason it is never part of the boundary above). That is reported with the same `mapping-path-missing` code a stale glob or a deleted file would produce: a node with real, non-empty `mapping:` and zero enforceable files needs to say so somewhere, the same way it would if the mapping pointed at a path that no longer exists.
