...
> **Note:** By default `yg check` writes no verdicts and never touches the lock, makes no reviewer calls, and needs no keys. When `auto_approve` is set to `deterministic` or `full` in `yg-config.yaml`, bare `yg check` behaves like `yg check --approve --only-deterministic` or `yg check --approve` respectively. Explicit CLI flags always override the config, and a triage view (`--top`, `--summary`, `--aspect`, `--details`) never fills. Under CI — the `CI` environment variable set to anything but empty, `0` or `false` — a committed `full` is held back: bare `yg check` fills nothing, not even script pairs, and says `auto-approve: full ignored — CI is set` on stderr (`deterministic` is not held back). CI scripts use explicit flags and are unaffected by `auto_approve`.
...
- **Either kind, when the rule's directory holds more than its rule files** — every other file there is folded in with the rule: a helper module `check.mjs` or `companion.mjs` imports, a table it ships. Their bytes decide the verdict as much as the rule file's own, so changing only a helper re-opens the rule. Left out: `yg-aspect.yaml`, `log.md`, a package rule's adaptation and its log, a generator's `provenance.json`, `node_modules`, and every file under a dot-named entry that is not code — a dot-named `.mjs`, `.js` or `.cjs` module runs like any other, so it is folded in (unless the repository's `.gitignore` ignores it, as it would a `.venv`, `.cache` or `.turbo`), while a `.DS_Store` differs from machine to machine and is not. The `drills/` corpus and a nested rule's own directory are left out too, except a file there that the rule's code names by a literal relative specifier (an `import`, `require` or `new URL(…, import.meta.url)`, followed from module to module; a specifier built at run time is not seen, only the exact on-disk spelling counts, and a symbolic link it names is refused with `aspect-source-symlink`, never followed): a helper is part of every rule that runs it, wherever it sits, while a drill case added to the corpus re-opens nothing. A module imported from outside the rule's directory is not folded in. For a reviewer rule this covers files the reviewer never sees: only `content.md` reaches the prompt as the rule, so another file beside it (a `guidance.md`) re-opens the rule when edited without changing what the reviewer reads — list it under `references:` to show it. Folded only when present: a rule directory holding nothing else hashes exactly as it always did.
...
However, when `auto_approve` is configured in `yg-config.yaml`, bare `yg check` may fill pairs automatically: `auto_approve: deterministic` behaves like `yg check --approve --only-deterministic`; `auto_approve: full` behaves like `yg check --approve`. Two cases stay read-only whatever it says: a triage view (`--top`, `--summary`, `--aspect`, `--details`) never fills, and under CI (the `CI` variable set) a committed `full` is held back — bare `yg check` fills nothing, neither reviewer nor script pairs, and says so on stderr, because a config-driven fill in CI would record fresh verdicts over a change nobody re-verified (an explicit `--approve` still fills; `deterministic` still fills the script pairs). CI scripts use explicit flags (`yg check --approve --only-deterministic`) and are unaffected by `auto_approve` — the CI-is-free-and-keyless guarantee holds.
...
| `yg aspect-test`, `yg drill`, `yg simulate` | **Yes** — the rule under test (`yg simulate`: the candidate's `check.mjs`, once per replayed commit, in a throwaway clone — it writes nothing to your repository but runs the code all the same): a script rule's `check.mjs`; for a reviewer rule, its `companion.mjs` (`yg aspect-test` only — `yg drill` records a rule that ships one as unsupported and runs nothing of it) and the source sent to the configured reviewer. |
| bare `yg check` with `auto_approve: deterministic` or `full` | **Yes** — it becomes the `--approve` form above. `auto_approve` is committed configuration, so a branch can switch it on. Under CI (the `CI` variable set) a committed `full` is held back and runs nothing; `deterministic` still runs every `check.mjs`. A triage view (`--top`, `--summary`, `--aspect`, `--details`) never fills and runs nothing. |
