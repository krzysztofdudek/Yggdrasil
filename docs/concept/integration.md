...
- **When starting from a high-level goal**, the agent loads context with
  `yg context --node` to get a node overview: aspects, flows, dependents, and metadata.
  Node descriptions and aspect content are written in the same vocabulary developers use
  in task descriptions. This gives the agent a structured reading phase before any design
...
- **After modifying**, the agent updates graph metadata, runs `yg check` (the structural gate),
  and then `yg approve` (the semantic verification gate). Approve runs reviewer checks: it verifies
  each aspect against source code. If no reviewer is configured, approve accepts any change
  (binary model) — a notice informs the user.
...
- “Design the payments module” → agent creates nodes, relations, aspects
...
The agent creates and edits graph files — YAML metadata (`yg-node.yaml`, `yg-flow.yaml`,
`yg-aspect.yaml`) and aspect content files (`content.md`). Aspects are the only Markdown in
the graph — they contain enforceable rules verified at approve time. Tools have no write
operations to the graph — they are readers and validators of semantic content. Tools do write
operational metadata (`.drift-state/`) for drift tracking; see the [Engine](engine) document.
...
Agent: creates yg-node.yaml with a relation to “payment/svc”
  ...
  → yg check: no errors
  → yg approve: aspect-violation “aspect 'no-direct-db-access' not satisfied in order-service.ts”
...
| Aspect content files (`content.md`)                            | Agent                      |
...
=== BEFORE ANY TASK ===
- yg context --node on relevant nodes
- READ phase: aspects (read content files — rules are inside),
  flows (read invariants), relations (check interfaces)
...
AFTER MODIFYING:
- Update graph metadata (per file, not batched)
- yg check — structural gate
- yg approve --node — reviewer verifies aspects
  (no reviewer configured → binary accept)
...
1. `yg context --node` on relevant nodes — node overview (aspects, flows, dependents)
2. `yg context --file <path>` — per-file details (aspect requirements to satisfy, consumed dependencies)
3. Modify source code — aspect requirements tell you what rules to follow
4. Update metadata → `yg check` → `yg approve --node`
...
By reading this file, the agent immediately knows what node types are allowed and what aspects
each node must prove. Tools read the same file to validate; the agent reads it to understand
what constraints apply to its choices.
...
- `yg check` — structural gate: broken references, coverage gaps. Fast and deterministic.
- `yg approve` — semantic gate: reviewer verifies aspects against source code. Self-teaching
  errors tell the agent exactly what to fix:
  - **aspect-violation:** an aspect is not satisfied by the source file — the agent
    must fix the code or update the aspect.

If no reviewer is configured, approve uses the binary model — any change triggers acceptance.
...
this project’s conventions. Tools translate project configuration into guidance at the moment
the agent needs it.
...
updates to that node's metadata. An automated analysis of the diff against the node's
node metadata can propose graph patches with high precision — changes that are clearly correct
can be auto-applied, while uncertain changes are flagged for review.
...
- **Graph updates are immediate, not deferred.** Subagents update graph metadata after
  each file, not as a final batch. Context is freshest immediately after writing code.
  Deferred updates produce shallow, low-value metadata.
- **External knowledge is absorbed, not referenced.** When a subagent works from external
  ...
  must be captured in the graph. The graph does not point to external sources — it
...
source file, does a corresponding graph node exist? If not, the work is
...
- The agent has `yg-config.yaml` with project configuration and initial node metadata
...
```text
empty → a few shallow nodes → growing coverage → deepening descriptions
        and aspects in response to bad outputs → mature graph
```
...
Characteristic: semantic memory (descriptions, aspects, constraints) survives refactoring even
...
details (aspect requirements to satisfy, consumed dependencies). Both output structured text. The context is typically 5,000-10,000 tokens regardless of project size. The agent
...
Nodes exist, but their descriptions say nothing meaningful. Context packages are technically
assembled but provide no real direction.

Defense: the self-calibrating granularity loop (shallow graph → bad output → human says
...
soft behavioral directives alone.
