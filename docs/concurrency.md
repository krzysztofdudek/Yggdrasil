...
## One fill at a time per repository
...
**Why.** A fill (a `yg check --approve` run) reads the recorded results once, then writes its own copy of them back as it fills. Two fills running side by side would each overwrite the other's copy, the one that finished last would win, and the verdicts of the other would be lost — paid reviewer verdicts included. So for the whole run, from that first read to its last write, a fill holds a lock file, `.yggdrasil/.yg-approve.lock`, which records the holder's process id, machine and start time; the operating system lets only one process create it. The portal's Approve button obeys the same lock: while a fill is running — started from the portal or from a terminal — it answers `409` instead of starting a second one. The lock is released when the run ends, including on Ctrl+C. A lock left by a run that crashed does not block you: when its process no longer exists on this machine, the next fill replaces it. The one case Yggdrasil cannot check is a holder on another machine sharing the directory; such a lock counts as abandoned after 12 hours, or you delete the file once you know that run is gone. A plain `yg check` on a repo with `auto_approve` turned on takes the same lock, because it fills too. A cost preview (`--dry-run`) writes nothing and takes no lock.

**What an interruption costs.** Results are written as the run goes, in small batches rather than by rewriting the whole record after every single result (which made a large first fill spend most of its disk traffic rewriting the same file). Each paid reviewer verdict is written before its slot takes the next pair; free script results are written at least every 256 results and at least once a second. Stopping a run with Ctrl+C (or `kill`) writes everything decided so far before the process exits. A hard kill that nothing can intercept (`kill -9`, a machine losing power) loses at most the free results of the batch being written and the one being gathered — at most two batches of 256 script results plus the few in progress at that instant, all re-computed for free by the next run — and the reviewer verdicts whose write was in flight at that instant. What was written is always a complete, readable file.

Parallelism *inside* a single run is handled for you: one fill fans out and writes its results safely. The free script rules run across your cores automatically; the reviewer's calls run one at a time by default — raise `parallel` in `yg-config.yaml` to fan those out too. The hazard is two separate `--approve` runs overlapping — not one busy run, however it is tuned.

## What is always safe to run concurrently

**Rule.** Read-only commands never conflict — run as many as you like, at the same time, including while a fill is in progress.

**Why.** They never write results. This covers plain `yg check` (without `--approve`), `yg structure`, `yg aspects`, and `yg log read`. One caveat: if the project has turned on the `auto_approve` setting in its config, a plain `yg check` runs a fill on its own — and therefore writes — so on such a repo it counts as a fill for the rule above, not a read-only command.
