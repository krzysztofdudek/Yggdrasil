...
- A subtree that is its own separate project — carrying its own nested \`.yggdrasil/\` graph, or its own \`.git\` (a checkout, submodule, or linked worktree) — is auto-skipped wherever a DIRECTORY or GLOB mapping entry SWEEPS it in as files a node enforces, is billed for, or reads (coverage, review pairs, fingerprints, dependency checks, type classification, rule reads including through a symlink into it, and ownership lookups by file), regardless of \`.gitignore\`. A mapping entry that names one file inside the separate project EXACTLY is a different, unaffected case — the node's own explicit ownership claim over that one path, left alone by every check above (whether that explicit claim is itself a valid mapping is \`file-mapping-nested-project\`'s own question). The one exception even for a sweeping entry: a mapping entry's own on-disk existence check stays silent as long as it resolves to real content somewhere, even content belonging to the separate project — a node whose directory/glob entries, as a WHOLE, resolve only into one is reported separately, as \`mapping-path-missing\` on a mapping that enforces nothing. A dependency directory (e.g. \`node_modules\`) is never skipped by name alone; exclude it explicitly via \`excluded\` if desired.
