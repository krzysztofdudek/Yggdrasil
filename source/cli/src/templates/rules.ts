...
BEFORE any task (brainstorming, design, planning, implementation):
  ...
  READ phase — collect constraints before designing:
    - Architecture: read yg-architecture.yaml to understand node type constraints (aspects, allowed parents/relations)
...
.yggdrasil/
  ...
  yg-architecture.yaml  ← node types with constraints: aspects, parents, relations
  model/                ← what exists: nodes, hierarchy, relations, file mappings
  aspects/              ← what must: cross-cutting requirements with claim-based anchors
...
- **Architecture:** \`yg-architecture.yaml\` defines node types with constraints. Every node's type must match an architecture type and satisfy its constraints.
  - \`aspects\` — required on every node of this type.
...
- **Mapping:** In \`yg-node.yaml\`, \`mapping:\` is a flat list of file/directory paths relative to repo root.
- **Ports:** Nodes can declare \`ports\` — typed entry points with aspect requirements. Consumers that use a port (via \`consumes\` on relations) inherit the port's aspects.
- **Effective aspects:** Aspects propagate from multiple sources. Agents don't manually compute — \`yg context\` shows \`required_aspects\` with sources. Understand: architecture requirement? Flow requirement? Parent inheritance? Own extra? Port consumption?
- **Aspect id = directory path** under \`aspects/\`. Each aspect has \`yg-aspect.yaml\` (defines claim-based anchors) + content \`.md\` files. No automatic parent-child — use \`implies\` explicitly.
...
- **\`node\`** — the target node with:
  ...
  - \`integration_aspects\` — aspects from consumed ports on target nodes (if any)
  - \`mappings\` — source file paths
