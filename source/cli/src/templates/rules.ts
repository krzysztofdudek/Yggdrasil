...
.yggdrasil/
  ...
  yg-lock.nondeterministic.json ← committed: LLM verdicts; written only by the CLI. Never hand-edit. On a merge conflict take ONE side wholesale, then run yg check --approve.
...
| \`yg check --approve --only-deterministic\` | Fill ONLY deterministic pairs (free, keyless), writing the gitignored cache; then report. The CI / pre-commit gate. Overrides \`auto_approve\`. |
...
| \`yg check --aspect <id>\` | Read-only: drill-in view — only the named rule's blocks, every member listed. |
...
| \`yg aspect-test --aspect <id> --node <path>\` | Diagnostic — run a check/reviewer live without touching the lock (\`--dry-run\` previews an LLM prompt; \`--files\` for ad-hoc and \`--check-determinism\`, both deterministic aspects only; \`--repeat N\` re-runs an LLM aspect N times to measure reviewer self-consistency, not correctness; \`--tier <name>\` re-runs the same pairs under a named reviewer tier — the dry-fit before a model swap) |
