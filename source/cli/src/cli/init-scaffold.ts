 ...
 *    - the roots decisions/ledger merge=union lines — those two stores are
 *      COMMITTED, append-only case law (promotions/mutes/dismissals, and
 *      hook-shaped marks) appended on many branches, the same rationale as the
 *      advise-decisions register above; a union merge keeps every branch's
 *      entries instead of forcing a conflict.
 *    - the roots model.json linguist-generated line — that snapshot is
 *      machine-written (regenerated by `index --full`, never hand-edited), the
 *      same rationale as the lock triad's own linguist-generated line above.
 *      (These three roots lines are unconditional scaffold, same as the rest of
 *      this list: they apply even to a project with no `roots:` config block —
 *      the files they describe do not exist yet in that case, and an attribute
 *      naming an absent file is harmless.)
...
const GITATTRIBUTES_LINES = [
  ...
  '/.yggdrasil/roots/decisions.jsonl merge=union',
  '/.yggdrasil/roots/ledger.jsonl merge=union',
  '/.yggdrasil/roots/model.json linguist-generated=true',
] as const;
 ...
 *    - `roots/.cache/`    — the roots engine's blob cache and build lock (rebuildable, sharded)
 *    - `roots/.state/`    — the roots engine's telemetry, session, and incident state (durable but local)
...
const YGGDRASIL_GITIGNORE_LINES = [
  ...
  '*.tmp',
  // Roots engine's blob cache and build lock (design §4, integration-design.md:122-165):
  // content-addressed, sharded, rebuilt free on the next `index` run; never committed.
  'roots/.cache/',
  // Roots engine's telemetry/session/incident state: local and durable (survives
  // `index --full`, unlike the cache above), but never committed — it is per-machine
  // observational state, not part of the mined model.
  'roots/.state/',
] as const;
