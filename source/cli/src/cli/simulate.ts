...
function realpathOfExistingPrefix(target: string): string | null {
  ...
  for (;;) {
    try {
      ...
      debugWrite(`[simulate] realpath of '${existing}' failed, walking up: ${(err as Error).message}`);
  ...
  program
    ...
    .action(async (candidate: string, opts: { node?: string; file?: string; maxCommits: string }) => {
      try {
        ...
        abortOnUnexpectedError(error, 'running yg simulate (nothing in the real repository was modified; all work happens in an isolated clone)');
      }
    });
}
