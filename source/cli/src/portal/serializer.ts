...
/*
 * The Yggdrasil brand mark (the world-tree logo), inlined here as a string constant. It is
 * defined in the serializer — its only consumer — rather than a separate module on purpose:
 * a portal-pipeline node may not import another (the architecture forbids the spider), and an
 * SVG carries the namespace xmlns (an http URL) that the FRONTEND offline guard would, within
 * its own scope, read as a network reference, so the markup stays on the backend where no URL
 * scan applies. The serializer base64-encodes it into a data:image/svg+xml URI injected into
 * the favicon link; the nav-rail brand reads that same URI back, so the logo ships fully inline.
 */
const LOGO_SVG = `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 400 400">
  <defs>
    <style>
      .st0 {
        fill: #f8d794;
...
const LOGO_DATA_URI = `data:image/svg+xml;base64,${Buffer.from(LOGO_SVG, 'utf8').toString('base64')}`;
...
export function renderStaticHtml(data: PortalData, assets: PortalAssets): string {
  ...
  return assets.shell
    .replace('__PORTAL_FAVICON_HREF__', () => LOGO_DATA_URI)
