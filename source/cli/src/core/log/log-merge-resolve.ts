...
export async function logMergeResolve(input: LogMergeResolveInput): Promise<LogMergeResolveResult> {
  ...
  let midMerge = false;
  const inProgress = sides === undefined ? await mergeInProgressHead(repoRoot) : null;
  if (inProgress !== null) {
    sides = { ours: 'HEAD', theirs: inProgress };
    midMerge = true;
  } else if (sides === undefined && !(await isMergeCommit(repoRoot, 'HEAD'))) {
    return {
      ok: false,
      error: {
