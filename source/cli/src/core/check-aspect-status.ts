  ...
  for (const drift of findStatusDrift(graph, lock)) {
    issues.push({
      ...
      messageData: {
        ...
        next: `Record why it moved: yg aspects log add --aspect ${drift.aspectId} --status ${drift.to} --evidence '<what justified it>' --reason '<why it moved>'. The next full yg check --approve otherwise writes the bare fact into that rule's log for you (--only-deterministic writes no committed file, so it leaves this warning standing).`,
      },
