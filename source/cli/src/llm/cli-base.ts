...
export abstract class CliAgentProvider implements LlmProvider {
  ...
  async verifyAspect(prompt: string): Promise<AspectResponse> {
    ...
    if (shell && !SHELL_SAFE_MODEL.test(this.model)) {
      return { satisfied: false, reason: `${this.binary} model '${this.model}' is not a model name (letters, digits, '.', '_', ':' and '-' only)`, errorSource: 'provider' };
    }
    ...
    return new Promise((resolve) => {
      ...
      try {
        ...
        debugWrite(`[${this.binary}] spawn threw: ${err instanceof Error ? err.message : String(err)}`);
        resolve({ satisfied: false, reason: `spawn error — is '${this.binary}' installed and on PATH?`, errorSource: 'provider' });
        return;
      }
