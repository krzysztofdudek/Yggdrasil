...
//   - legacy generated rules files  — `.yggdrasil/agent-rules.md`, `.cursor/**`
//                                     (the RETIRED per-platform installers'
//                                     output; auto-generated, never hand-edited).
//                                     Kept as an exclusion because a repo that
//                                     has not yet migrated to the universal
//                                     install — including this repo's own
//                                     example fixtures, until the dogfood task
//                                     regenerates them — may still carry these.
//   - .clinerules/yggdrasil.md      — wholly generated by the CURRENT universal
//                                     installer, never hand-edited, same
//                                     reasoning as the legacy mirrors above.
//                                     (AGENTS.md/CLAUDE.md are NOT exempted:
//                                     they carry real user content beyond the
//                                     generated digest block/import line, and
//                                     neither falls under SCAN_DIRS/SCAN_FILES
//                                     at the repo root anyway — only nested
//                                     copies under `examples/` would ever be
//                                     walked here.)
 ...
 * one of these. Generated rules files are reproduced from rules.ts / digest.ts
 * (the real source of truth) and may legitimately carry historical text;
 * per-node log.md is append-only history, not docs.
 */
function isExcludedPath(rel: string): boolean {
  ...
  if (rel === '.yggdrasil/agent-rules.md') return true; // legacy generated (retired installer)
  if (rel.startsWith('.cursor/')) return true; // legacy generated rules (retired per-platform installer)
  if (rel.endsWith('.cursor/rules/yggdrasil.mdc')) return true;
  if (rel.endsWith('.clinerules/yggdrasil.md')) return true; // current universal installer's generated mirror
