 ...
 * deterministic side, part 1 of 2: what the lock ends up holding — det-first
 * ordering and the det gate (enforced det skips LLM, keyed by detGateKey),
 * positive closure, the log gate, GC and the canonical rewrite, incremental
 * writeLock, and the implies-cycle abort that leaves every verdict untouched.
 * Part 2 (fill-det-execution.test.ts) covers how a check runs and reports:
 * observations and taint, the per-file contract, notices, dry-run and
 * --only-deterministic.
 *
...
describe('detGateKey cross-contamination — real fill run, componentless pairs', () => {
  it('a cached refusal on one file does not stop the LLM fill of an unrelated file matching the same type', async () => {
    ...
    await runFill(await loadGraph(dir), { ...IO, coverageVisibleFiles: visibleFiles, write: w.write, emitIssue: w.emitIssue });
...
describe('log gate (§9)', () => {
  it('fires on a fingerprint change with no fresh entry (all-or-nothing: aborts, approves nothing, node does NOT close)', async () => {
    ...
    await expect(
      runFill(graph, { ...IO, coverageVisibleFiles: null, write: w.write, emitIssue: w.emitIssue }),
...
describe('an implies cycle aborts approval before anything is collected', () => {
  it('leaves every recorded verdict untouched, including one an ordinary run would prune', async () => {
    ...
    await expect(
      runFill(graph, { ...IO, coverageVisibleFiles: null, write: () => {} }),
    ).rejects.toBeInstanceOf(FillGatingError);
