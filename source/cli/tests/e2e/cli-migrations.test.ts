...
describe.skipIf(!distExists)('CLI E2E — schema migrations (version-guard + idempotency)', () => {
  ...
  // (src/migrations/index.ts: `MIGRATIONS` now contains the to-5.1.0 and to-6.0.0 entries).
  ...
  it('M2: upgrading an already-v5 repo twice is a no-op — config byte-identical between runs', () => {
    ...
    try {
      // First upgrade on the v5 fixture: nothing to migrate (6.0.0 is the
      ...
      expect(afterSecond).toContain('version: "6.0.0"');
  ...
  it('M5: a config version newer than the CLI supports blocks `yg check` and `yg tree` (exit 1)', () => {
    // CLI_SUPPORTED_SCHEMA in graph-loader.ts is 6.0.0; a config at 99.0.0 must
    ...
    try {
      ...
      expect(check.all).toContain('max: 6.0.0');
      ...
      expect(tree.all).toContain('max: 6.0.0');
  ...
  it('M7: `yg init --upgrade` splits a legacy yg-lock.json into the triad, preserving every verdict by kind and gitignoring the cache', () => {
    ...
    try {
      ...
      // The gitignored cache was added to .yggdrasil/.gitignore, and the version advanced
      // to 6.0.0 (the to-5.1.0 migration's split step runs first, then the to-6.0.0
      // migration carries it the rest of the way — both are real transforming
      // migrations now, so no version-lift fallback is involved).
      expect(readFileSync(path.join(ygg, '.gitignore'), 'utf-8')).toContain('.yg-lock.deterministic.json');
      expect(readFileSync(configPath(dir), 'utf-8')).toContain('6.0.0');
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
