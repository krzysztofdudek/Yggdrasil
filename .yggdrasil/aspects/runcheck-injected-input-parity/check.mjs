...
const ISSUE_TRANSFORM = new Map([
  [
    'changeScope',
    "carries which of this run's obligations the current change is accountable for, plus the name it was measured against. Scope decides which issues a run reports as blocking and which it re-codes as pre-existing, so it alters the issue set by definition and can never be side-effect-only. STATE, as of the classification step landing: on THIS repository's seam the member is now consumed in the recognized whole-list-rewrite shape, so deriveIssueTransformKeys carries it and the sweep below skips this entry as redundant — it changes no verdict here, and the required-key set is identical with or without it. The entry is kept deliberately, because it is also the ONLY subject the fixture corpus has for this map's three failure branches: the honored declared-ahead-of-its-consumer window (runcheck-parity), the stale entry naming a member a seam never declared (runcheck-parity-drift), and the entry that outlives its window once a body reads the member in an unrecognized shape (runcheck-parity-unrecognized). A fixture cannot introduce a map entry — the map is global to this rule — so an empty map leaves every one of those branches untested. Deleting this line therefore does not tidy a redundancy; it deletes the test coverage of four ruleLevel refusals. Delete it only together with a replacement subject for those cases.",
  ],
]);
