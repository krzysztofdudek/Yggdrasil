...
| `roots-import-boundary` | over | The allowlist is a fixed, curated set of today's known-legitimate crossings, not an exhaustively-derived closed surface — a genuinely new, legitimate engine dependency the allowlist has not caught up with is falsely flagged. Two narrower gaps sit alongside the over-approximation, stated here rather than left silent: (1) a dynamic `import()`/`require()` whose argument is computed or interpolated (not a string/no-substitution-template literal) is not a static fact this check can read and is silently skipped — every statically-resolvable form, static or dynamic, IS covered; (2) the check only ever reads a core file's own direct specifier text, so an allowlisted module that itself re-exports something outside the allowlist (e.g. an allowlisted helper adding `export * from '../core/graph-loader.js'`) silently widens every core file's real reachable surface, invisibly to this check, because the disallowed specifier never appears in a core file at all — inherent to any per-file import fence, not a defect in this one. |
