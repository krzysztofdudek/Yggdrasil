...
## [2026-07-31T20:54:59.752Z]
A companion.mjs that reads or returns a path this graph excludes was rejected with the same undifferentiated error a genuinely undeclared path gets, so the printed advice told an agent to declare a relation that could never satisfy a read the graph has already ruled out. The rejection now carries which of the two causes applies — worked out once at the shared read guard instead of re-derived here — so an excluded companion path is reported as excluded by design, naming which of the two exclusion sources matched, instead of a relation to declare.
