...
2. **Config-dependent checks** (if !configError): checkNodeTypes, checkAspectsDefined, checkAspectIds, checkAspectIdUniqueness, checkImpliedAspectsExist, checkImpliesNoCycles, checkRequiredAspectsCoverage, checkRequiredArtifacts, checkInvalidArtifactConditions, checkContextBudget, checkHighFanOut, checkMissingDescriptions
...
**E050 renamed from missing-required-aspect to dangling-aspect-ref (v4):** The old E050 checked if mapping groups had required aspects. With mapping groups gone and aspect verification replacing regex proofs, E050 was repurposed as a general structural check: any aspect referenced by a node, flow, or architecture type must have a corresponding directory in `aspects/`. This subsumes the old E003 (unknown-aspect) and E035 (missing-required-aspect).
...
**E037/E040/E041/E054 removed (v4):** E037 (regex anchor-not-found), E040 (anchor-not-realized), E041 (unknown-anchor-type) were all part of the regex proof system which was replaced by LLM aspect verification. E054 (unexpected-aspect on mapping group) was removed with mapping groups. These validations now happen at approve time via LLM (E055/E056), not at check time.

**E053 rewritten for per-port validation (v4):** The old E053 checked global integration_aspects. The new E053 validates per-consumed-port: if node A consumes port X of node B, and port X requires aspect Y, aspect Y must exist in the graph. This is structural only — semantic verification (does A's code satisfy Y's requirements?) happens at approve via E055.
