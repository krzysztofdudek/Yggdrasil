...
**required_aspects and ports use computeEffectiveAspects when available:** In toContextMapOutput, when graph.architecture exists, we compute effective aspects (regular and ports) using computeEffectiveAspects, which includes architecture constraints, parent inheritance, own declarations, and flow participation, all with implies expansion. This provides complete aspect coverage. We then map each aspect ID to a human-readable source string (e.g., "architecture (type: service)", "own declaration", "flow 'checkout'"). Chose computeEffectiveAspects over collectEffectiveAspectIds because it handles ports separately and integrates architecture-level constraints. Falls back to collectEffectiveAspectIds (with source "collected from node and flows") when architecture is unavailable, maintaining backward compatibility with test fixtures and old-style graph loads. The source information (in RequiredAspectRef) helps agents understand aspect provenance — architecture constraints vs. flow obligations vs. node extras.
