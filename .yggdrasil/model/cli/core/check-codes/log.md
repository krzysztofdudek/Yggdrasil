...
## [2026-08-13T05:00:06.387Z]
The two committed graph files a fixed-input finding is judged by are now spelled the same way a changed file is, by importing that spelling rather than repeating it. Written without the graph directory in front they could never match anything a change touched, and because that rule outranks every other way of identifying a finding, such a finding would have been judged solely by an input it can never see.
## [2026-08-13T06:34:24.131Z]
The map from an outside-changes code back to the code it stands for is now derived here, beside the function that builds the suffix, and exported. A renderer needs to strip that suffix to describe such a finding the way it describes the finding it mirrors; without a derived answer it either slices the string itself — a second spelling of the suffix, free to drift from the one true one — or has no case for these codes at all and prints the raw code to a person, which is how an internal identifier ends up on screen. The forward and reverse directions are built from the same set in one expression, so neither can drift from the other or from the set as it grows.
## [2026-09-06T16:31:55.708Z]
...
## [2026-09-11T19:20:34.633Z]
The comment over the port-contract codes claimed the ports-and-relations knowledge topic documented the port code set, while that set silently omitted the one port code that is a warning rather than a gate. A reader auditing the registry for completeness would have concluded the warning did not exist, or that it was missing from the structural set by oversight. The comment now states why it is absent — a non-blocking code has no place in a set whose membership means 'blocks the gate' — and names it, so the omission reads as a decision instead of a gap.
## [2026-09-12T06:10:53.619Z]
...
## [2026-09-24T13:49:33.915Z]
config-unknown-key is a structural, blocking finding, and every code a configuration that did not load is reported under (config-invalid and the specific ones such as config-tier-unknown-key) is now recognised as one failure. The specific codes used to rank below the unmapped files and missing reviewer that they caused, so next: sent the user to fix symptoms.
## [2026-09-24T14:30:20.880Z]
The unverified causes gain keyed-by-earlier-release, ordered right after stale among the pairs one recording run can still fill: a script verdict whose inputs did not move but which an earlier release (or an earlier parser grammar) keyed differently must be named as the upgrade it is, so the report can give it its own subject and the free lane as its fix instead of blaming a source edit.
## [2026-09-25T17:43:43.963Z]
Type-only imports now count as dependencies between components, the same as value imports. The unverified-cause vocabulary moved to the model layer beside the check issue that carries it, so modules the orchestrator calls can name an issue without a cycle back to it; this module re-exports it under its old name.
## [2026-09-26T02:52:34.701Z]
An approving fill used to run over a node log that still held git conflict markers, a rewritten history or an unparseable body, and closure then recorded the node's baseline over it. Those three log codes now form their own approve-gating set. They are kept apart from the structural gating set on purpose: the report treats them as scoped codes that progressive mode may show as warnings, and a scoped code must never be a structural gating code, yet the fill applies them unnarrowed, exactly as its log-entry gate does. Two new structural codes were registered as well: one for a component the graph walk never reaches, and one for a rule file carrying a key its schema does not accept.
## [2026-09-26T03:36:05.158Z]
The comment on the log-state approve gate now says that a cost preview is exempt as well: a dry run records no baseline, so stopping it over an unsettled log would only hide the price of the run the user is weighing.
