...
Orchestrates three concerns: the three-axis change detection gate, LLM-powered reviewer verification (E055/E056), and batch approval for cascade drift. Batch mode (`--aspect`, `--flow`, multiple `--node <paths...>`, or parent node without mapping) finds all E021 cascade nodes caused by the specified entity (or approves the listed nodes) and processes them in parallel.
