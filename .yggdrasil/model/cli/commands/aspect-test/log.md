...
## [2026-07-31T01:47:38.795Z]
Added a --file addressing mode so an aspect can be tested directly against a file that is enforced by its architecture type alone, with no owning component: it refuses a path that already has a component (pointing at --node instead), refuses one that does not classify to exactly one architecture type, and otherwise runs the deterministic check with an architecture-derived read allowance or resolves the LLM pair and prints its prompt or verdict exactly as --node does. This closes the gap where the paid review tier had no way to preview or run a rule against a file with no component to attach to.
## [2026-07-31T03:09:56.356Z]
Extracted the --file target-classification logic (coverage-exclusion, single-type classification, the nodeless type-coverage lookup, and the nodeless architecture-reach computation) into its own engine node, cli/core/aspect-test-file-target, restoring this node's relation ceiling from 24 back to 21. The command layer keeps only the two checks that need command-only access: the on-disk existence probe and the ownership check via owner.ts's single-sourced findOwner, which an engine node cannot legally call.
## [2026-07-31T14:13:49.824Z]
The --file ownership pre-check used the raw, text-only owner lookup, so a file a mapping merely swept in but an exclusion removed from enforcement was refused as "has a component of its own" and pointed the caller at a component whose rules never actually run against it, instead of falling through to the classification path that already has the correct excluded-file refusal. It now goes through the same exclusion-aware wrapper the owner and context commands already use, so an excluded file reaches the right refusal instead of a false one.
## [2026-08-02T22:56:00.575Z]
aspect-test --files with a path that does not exist now reports it plainly, the same what/why/next --file already gives for the identical mistake, instead of letting a raw filesystem error reach the generic unclassified-error funnel and invite a spurious bug report for a typo.
## [2026-08-03T00:22:30.039Z]
A directory or an unreadable file passed to --files reached the AST runner as a raw filesystem error (EISDIR or EACCES) and printed the generic "this is a bug, please file an issue" message, misreporting an ordinary mistake as an internal defect. On --file, an unreadable file passed the old existence-only probe, was then silently treated as empty content by the structure runner, and printed a false "satisfied" verdict for content nobody actually checked -- worse than an error. Both flags now share one up-front probe that classifies a bad path as missing, not-a-file (a directory or other non-regular entry), or unreadable, and answer with a plain what/why/next naming the actual problem for each case, before either the runner or the type classifier ever sees the path. A broken symlink already fell cleanly into the missing case and needed no change.
## [2026-08-07T11:03:13.801Z]
Follows the prompt no longer carrying a component's description: the diagnostic that replays a rule against a component or a file assembles the same prompt the real reviewer does, so it stops passing one too. Keeping it here would have made the diagnostic show a prompt the reviewer never sees.
## [2026-09-23T19:26:21.673Z]
The diagnostic run stopped at a reviewer that could not run with the same generic endpoint-and-credentials sentence the fill used, hiding the provider own cause. It now prints the provider reason (binary not found and how to install it, no API key and which variable to set, server not answering) and points at debug: true for the full output, matching what yg check --approve reports.
## [2026-09-23T19:38:12.399Z]
A stability run that produced no verdict was reported as a bare line on stderr, unlike every other infrastructure report in the command, so it carried neither why the run was left out of the count nor where to look for the cause. It now goes through the structured what-why-next message and names the debug switch.
## [2026-09-23T22:03:02.760Z]
...
## [2026-09-24T01:18:18.787Z]
Diagnostic runs now record a hash of the exact prompt they judged, so rule-health analysis can tell a split vote on one input (an ambiguous rule) from a refusal and a later approval on two different inputs (a bug that was fixed). Without it the ordinary fix loop was reported as rule ambiguity and nominated a rule for sharpening that had judged consistently. The repeat mode also compares the file:line locations the refusals cite, because a real reviewer refused every run of a borderline rule while naming a different list of violations each time, and the verdict ratio hid that the list the adopter would fix was not stable; overlapping line ranges count as one location so the same text cited as 1-5 and 2-4 is recognised as agreement. The consensus vote suffix counts verdict votes only, matching the new aggregation.
## [2026-09-24T02:09:08.570Z]
The note printed when a deterministic rule is run against a node it is not attached to is agent-visible guidance, not report output, so it now goes through the structured what, why and next like every other diagnostic of this command: what happened (an ad-hoc run), why it matters (yg check will produce no verdict for the pair) and the next step (attaching the rule, which is the user's architecture decision). It used to be one hand-written line that named the consequence but no way forward.
## [2026-09-24T07:23:40.361Z]
...
## [2026-09-24T15:11:23.261Z]
Two release lines met in this command's help text: one dropped the '(s)' plural shorthand, the other renamed deterministic aspects to script rules and the architecture-type-only file to a type-covered file. The help now carries both: plain plurals and the glossary's words, so a reader of --help meets the same terms the Glossary defines.
## [2026-09-25T21:13:50.088Z]
aspect-test now refuses an unknown rule id with the shared aspect-not-found error. An unknown rule id used to be refused in about six different wordings under two codes depending on the command, so an agent could not recognise the one situation or learn one remedy.
