...
- **Whole-flow proof, not just units.** Every functional change lands with an end-to-end test that drives the complete user flow through the public surface (spawn the built binary; browser surfaces through real Playwright + Chromium) and asserts the flow's observable outcome — unit tests alone are insufficient evidence, however thorough.
